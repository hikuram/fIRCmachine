...
* **`orbmol+alpb` (MLIP + xTB solvation-delta correction):**
  ...
  * By default, `TBLITE_METHOD = "hybrid"` temporarily uses the highly robust GFN1-xTB during the **entire initial path search phase** (Input Optimization, DMF, NEB, SCAN) to prevent crashes. It then automatically restores the effective production Delta-ML backend to GFN2-xTB for subsequent single-point energy evaluations, TS optimizations, and downstream steps.
